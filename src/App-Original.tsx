...
function TemplateEditor({ 
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        const newTemplate: TemplateData = { 
          ...
          frenteImg: template?.frenteImg || null,
          versoImg: template?.versoImg || null,
          frenteCampos: template?.frenteCampos || [],
          versoCampos: template?.versoCampos || []
        };
  ...
  const handleFieldMouseDown = (e: React.MouseEvent<HTMLDivElement>, fieldId: string) => {
    if (isAddingField || !template) return;
    
    e.preventDefault();
    e.stopPropagation();
    
    const field = (activeSide === 'frente' ? template.frenteCampos : template.versoCampos).find(f => f.id === fieldId);
    if (!field) return;
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!draggedField || !template) return;
    ...
    const constrainedX = Math.max(0, Math.min(newX, 600 - 100)); // Assuming min width 100
    const constrainedY = Math.max(0, Math.min(newY, 400 - 30)); // Assuming min height 30
    
    const campos = activeSide === 'frente' ? template.frenteCampos : template.versoCampos;
    const updatedCampos = campos.map(field => 
      field.id === draggedField 
        ? { ...field, x: constrainedX, y: constrainedY }
        : field
    );
    
    const newTemplate = {
      ...template,
      [activeSide === 'frente' ? 'frenteCampos' : 'versoCampos']: updatedCampos
    };
            ...
            {(activeSide === 'frente' ? template.frenteCampos : template.versoCampos).map((field) => (
              <div
                ...
                className={`absolute border-2 ${
                  ...
                  isAddingField ? 'cursor-not-allowed' : draggedField === field.id ? 'cursor-grabbing' : 'cursor-grab'
                }`}
                style={{
                  left: `${field.x}px`,
                  top: `${field.y}px`,
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (isAddingField) {
                    // Allow adding field at click position when in adding mode
                    const rect = e.currentTarget.parentElement?.getBoundingClientRect();
                    if (rect) {
                      addField(e.clientX - rect.left, e.clientY - rect.top);
                    }
                  }
                }}
              >
...
function CarteirinhaGenerator({ 
  ...
  onTemplateSelect?: (template: TemplateData | null) => void;
